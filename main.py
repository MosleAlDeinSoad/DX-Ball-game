...
def draw(s):
    '''
        ...
        4 5
    '''
    digit=['01540','04','013245','0132354','02315','102354','235401','015','3245102','51023']
...
class Paddle:
    def __init__(self,cx,ty,w,h,color=(0,1,0)):
        self.cx=cx # center
        self.ty=ty # top
        self.w=w
        self.h=h
    ...
    def draw(self):
        glColor3f(*self.color)
        glBegin(GL_POLYGON)
        glVertex2f(self.cx-self.w/2,self.ty)
        glVertex2f(self.cx+self.w/2,self.ty)
        glVertex2f(self.cx+self.w/2,self.ty+self.h)
        glVertex2f(self.cx-self.w/2,self.ty+self.h)
        glEnd()


bricks=[]
paddle=Paddle(WIDTH//2,0.95*HEIGHT,100,11)
balls=[Ball(9,WIDTH//2,paddle.ty-10) for _ in range(n)]
...
def frame(z):
    ...
    if gameover and time()-astart>10:
        return
    if z%FPS==0:
        z=0
        generateBricks()
    ...
    for bl in balls:
        ...
        if (bl.cx-nx)**2+(bl.cy-ny)**2<bl.r**2:
            if nx>=paddle.cx: bl.v[0]=abs(bl.v[0])
            else: bl.v[0]=-abs(bl.v[0])
...
def mouseMove(mx,my):
    if mx-paddle.w/2<0 or mx+paddle.w/2>WIDTH:
        return
    paddle.cx=mx
